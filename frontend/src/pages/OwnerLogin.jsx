...
import { LogIn, Mail, Lock, AlertCircle, ArrowRight, Building2, Eye, EyeOff } from 'lucide-react';
      ...
      <div className="w-full max-w-md">
        <div className="flex justify-center mb-4">
          <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
            <Building2 className="h-3.5 w-3.5" /> I have parking Access
          </span>
        </div>

        <h1 className="text-3xl font-extrabold text-slate-900 text-center mb-2 font-sans">Rent out Your Space</h1>
        <p className="text-slate-400 text-center text-sm mb-8">Sign in to manage listings &amp; track earnings</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link to="/forgot-password?role=owner" className="text-xs text-emerald-600 hover:underline font-medium">Forgot password?</Link>
              </div>
              <div className="relative">
            ...
            >
              {loading ? (
                ...
                <>Sign In (I have parking) <ArrowRight className="h-4 w-4" /></>
              )}
            </button>
          </form>
          ...
          <p className="text-center text-sm text-slate-500 mt-6">
            ...
            <Link to="/owner/register" className="text-emerald-600 font-bold hover:underline">Sign up free</Link>
          </p>
          ...
          <div className="border-t border-slate-100 mt-5 pt-4 text-center">
            <Link to="/seeker/login" className="text-xs text-slate-400 hover:text-slate-600 font-semibold">
              Are you a Parking Seeker? (I want parking) Click here to Login →
            </Link>
          </div>
        </div>
      </div>
