...
import { User, Mail, Phone, Lock, AlertCircle, CheckSquare, Square, Building2, Eye, EyeOff } from 'lucide-react';
...
const OwnerRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await signupForRole('owner', name, email, password, contact);
      ...
      <div className="w-full max-w-md">
        <div className="flex justify-center mb-4">
          <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
            <Building2 className="h-3.5 w-3.5" /> I have parking Sign Up
          </span>
        </div>

        <h1 className="text-3xl font-extrabold text-slate-900 text-center mb-8 font-sans">
          Create I have parking Account
        </h1>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                ...
                <a href="#" onClick={e => e.stopPropagation()} className="text-emerald-600 hover:underline font-semibold">Privacy Policy</a>, and{' '}
                <a href="#" onClick={e => e.stopPropagation()} className="text-emerald-600 hover:underline font-semibold">Host Rules</a>.
              </p>
            </div>
            ...
            >
              ...
              ) : 'Register (I have parking)'}
            </button>
          </form>

          <p className="text-center text-sm text-slate-500 mt-6">
            Already have an account?{' '}
            <Link to="/owner/login" className="text-emerald-600 font-bold hover:underline">Sign in</Link>
          </p>
          ...
          <div className="border-t border-slate-100 mt-5 pt-4 text-center">
            <Link to="/seeker/register" className="text-xs text-slate-400 hover:text-slate-600 font-semibold">
              Are you a Parking Seeker? (I want parking) Register Here →
            </Link>
          </div>
        </div>
      </div>
